...
#### Этап 1 — Подготовка данных и архитектура Бейзлайна
На этом этапе формируется фундамент системы и создается простая модель для сравнения.

**Таблица данных для обучения:**

| Название данных | Источник (витрина) | Роли | Статус (EDA) |
| :--- | :--- | :--- | :--- |
| Продажи (Target) | `DATAMARTS_SALES_PER_DAY` | DE/DS | Проверено (2.67 млрд строк) |
| Остатки (Stocks) | `ERP_STOCKS_DAILY` | DE/DS | Проверено (OOS коррекция) |
| Справочник товаров| `DIM_PRODUCTS` | DS | Проверено (выделен Fresh) |
| Праздники/События | `DIM_CALENDAR` | DS | Проверено (пики спроса) |

*   **Определение целевой переменной:** Ежедневный объем продаж товара в штуках/кг в конкретном магазине (`target_qty`). При `Stock=0` значение корректируется на основе среднего спроса (восстановление упущенного спроса).
*   **Техника Бейзлайна:** Метод скользящего среднего за последние 7 дней (MA-7), умноженный на коэффициент сезонности аналогичной недели прошлого года.
*   **Метрика:** WAPE (Weighted Average Percentage Error). Ожидаемый WAPE для бейзлайна — 35-45%.
*   **Результат этапа:** Очищенная витрина данных в ClickHouse и готовый расчет бейзлайна для всех 500 магазинов.

---

#### Этап 2 — Построение прогнозных моделей (MVP)
Переход от статистики к машинному обучению для учета сложных факторов (промо, бренды).

*   **Формирование выборки:** 
    *   *Train:* 12 месяцев истории (2023 год).
    *   *Validation:* Последние 14 дней доступных данных (Out-of-time validation).
*   **Техника решения:** Глобальная регрессионная модель **CatBoost**. Обучается на всем массиве данных, используя ID магазина и ID товара как категориальные признаки.
*   **Feature Engineering:** Создание лагов (продажи 1, 7, 14, 28 дней назад) и признаков бренда/типа упаковки. EDA показало, что лаг 7 и 14 имеют максимальную корреляцию с целевой переменной.
*   **Горизонт и гранулярность:** Прогноз на 14 дней, гранулярность Товар-Магазин-День. Пересчет ежедневный.
*   **Риски:** Проблема переобучения на данных одного крупного магазина. *Решение:* Использование регуляризации и группировки по категориям товаров.
*   **Результат:** ML-модель с целевым WAPE <= 20-25% для ключевых категорий.

---

#### Этап 3 — Оптимизация и бизнес-ограничения (Fresh-логика)
Адаптация математического прогноза под суровые требования ритейла (срок годности).

*   **Бизнес-правило:** Если `Shelf_Life < 5 дней` (категория Fresh), система применяет алгоритм дробления.
*   **Техника:** Если суммарный прогноз на 7 дней превышает емкость полки или риск списания по сроку годности велик, объем заказа принудительно делится на ежедневные поставки.
*   **Метрики:** Снижение коэффициента списаний (Waste Ratio) относительно бейзлайна.
*   **Риски:** Модель может заказать слишком мало, боясь списаний. *Решение:* Введение асимметричной функции потерь (штраф за дефицит выше, чем за избыток).
*   **Результат:** Алгоритм «умного заказа», который минимизирует списания молока и хлеба.

---

#### Этап 4 — Тестирование и подготовка пилота
Проверка модели «в бою» и интеграция с пользователями.

*   **Backtesting:** Прогон модели на исторических данных за прошлый год. Сравнение прогноза ИИ с реальными фактами продаж.
*   **Бизнес-проверка:** Совместный с Product Owner анализ 100 самых популярных товаров. Проверка: «Не заказывает ли модель аномально много под конец срока годности?».
*   **Интеграция:** Разработка API, которое передает рекомендацию ИИ в интерфейс менеджера магазина.
*   **Результат:** Отчет о точности (Backtest Report) и запущенный интерфейс подтверждения заказов для 10 пилотных магазинов.

---

#### Этап 5 — Закрытие технического долга и автоматизация
Превращение прототипа в надежную промышленную систему.

*   **Инфраструктура:** Переход от ручных скриптов к **Airflow DAGs**, которые управляют последовательностью: `Загрузка -> Очистка -> Обучение -> Инференс`.
*   **Мониторинг Data Drift:** Реализация автоматического сравнения распределений продаж. Если EDA за текущую неделю показывает резкий сдвиг (например, из-за смены сезона), система подает сигнал на переобучение.
*   **Технический долг:** Оптимизация SQL-запросов к 2.67 млрд строк (индексация и партиционирование таблиц в ClickHouse).
*   **Результат:** Полностью автоматизированный конвейер (Pipeline), работающий без участия DS, с системой алертов о потере точности.
