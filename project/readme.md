...
Центральный этап: превращение сырых данных (2,67 млрд строк) в витрины для моделирования.

**Таблица данных для этапа:**

| Название данных | Источник (витрина) | Требуемые роли | Качество (EDA) |
| :--- | :--- | :--- | :--- |
| Продажи (Target) | `DATAMARTS_SALES_PER_DAY` | DE/DS | + (нужна OOS-коррекция) |
| Остатки (Stocks) | `ERP_STOCKS_DAILY` | DE/DS | + (обнаружены дыры в данных) |
| Справочник товаров| `DIM_PRODUCTS` | DS | + (есть срок годности) |
| Промо-календарь | `MARKETING_PLAN` | PO/DS | +/- (неполный архив) |

*   **Бейзлайн:** Агрегация продаж до уровня Товар-Магазин-День. Заполнение пропусков нулями.
*   **MVP:** Сложная предобработка. Применение **OOS-коррекции**: если остаток 0, целевая переменная заменяется на прогноз спроса (восстановление упущенной выручки).
*   **Результат:** Готовые витрины в ClickHouse.

#### Этап 2 — Построение прогнозных моделей
Создание математического ядра системы.

*   **Бейзлайн:** 
    *   *Техника:* Статистический метод (MA-7) — среднее за 7 дней с сезонным коэффициентом.
    *   *Выборка:* Скользящее окно за последние 14 дней.
    *   *Результат:* "Наивный" прогноз без учета цен и промо.
*   **MVP:** 
    *   *Техника:* Глобальная модель **CatBoost** (обучается на всей сети сразу). 
    *   *Выборка:* 12 мес. — обучение, последние 14 дней — валидация (Time-series split).
    *   *Гранулярность:* Товар-Магазин-День, пересчет ежедневно в 03:00.
    *   *Feature Engineering:* Лаги (1, 7, 14, 28), категориальные признаки (бренд, формат точки), флаг промо.
    *   *Метрики:* WAPE <= 20% (в сравнении с WAPE бейзлайна ~40%).
    *   *Риски:* Огромный объем данных замедлит обучение. *Меры:* Использование GPU-кластеров.



#### Этап 3 — Оптимизация и бизнес-ограничения (Fresh-логика)
Трансформация прогноза в конкретный заказ.

*   **Бейзлайн:** 
    *   *Техника:* Фиксированный страховой запас (например, +10% к прогнозу). Никакого учета сроков годности.
*   **MVP:** 
    *   *Техника:* Алгоритм **"Smart Splitter"**. Для товаров со сроком годности <5 дней заказ дробится на ежедневные поставки, чтобы суммарный запас на полке не превышал прогноз реализации на 2 дня.
    *   *Метрики:* Снижение списаний (Waste) на 15% относительно бейзлайна.
    *   *Риски:* Конфликт ИИ с логистикой (грузовик не может возить по 2 пакета молока). *Меры:* Добавление ограничения на минимальную партию заказа.
*   **Результат:** Логика формирования заказов, снижающая потери Fresh-категории.



#### Этап 4 — Тестирование и подготовка пилота
Проверка гипотез на исторических и реальных данных.

*   **Бейзлайн:** 
    *   *Техника:* Сравнение точности среднего значения с реальными продажами за прошлый месяц.
*   **MVP:** 
    *   *Техника:* Полноценный **Backtesting** на 12 месяцах истории. Оценка экономического эффекта: сколько денег мы бы сэкономили на списаниях, если бы работал ИИ.
    *   *Интеграция:* Подключение модели к UI менеджера. Прогноз ИИ выводится как "Рекомендация".
    *   *Бизнес-проверка:* Совместная приемка с Product Owner топ-100 позиций.
    *   *Результат:* Подтвержденная точность и готовность 10 магазинов к старту пилота.



#### Этап 5 — Закрытие технического долга и автоматизация
Превращение эксперимента в стабильный продукт.

*   **Бейзлайн:** 
    *   *Техника:* Ручной запуск Python-скриптов аналитиком по утрам.
*   **MVP:** 
    *   *Техника:* Автоматизация через **Airflow**. Настройка мониторинга **Data Drift**: если структура спроса в магазинах резко меняется (EDA-контроль), модель автоматически уходит на переобучение.
    *   *Оптимизация:* Индексация ClickHouse для моментальной отдачи прогноза.
    *   *Риски:* Сбой в Airflow сорвет заказ. *Меры:* Настройка алертов в Telegram и "запасного" бейзлайна на случай падения ML.
*   **Результат:** Промышленный пайплайн, работающий в автоматическом режиме.
...
#### Описание сервисов и их назначения

1.  **Ingestion Service (Сервис загрузки)**
    *   **Назначение:** Прием высоконагруженного потока данных (до 10 млн транзакций в сутки) из внешних систем магазинов (POS). 
    *   **Функции:** Первичная валидация данных, дедупликация и преобразование JSON-потоков в пакеты (batches) для записи в распределенное хранилище.

2.  **ClickHouse Cluster (Распределенное хранилище)**
    *   **Назначение:** Аналитическая база данных для хранения и сверхбыстрой обработки 2,67 млрд записей.
    *   **Функции:** Выполнение тяжелых агрегационных запросов для подготовки обучающих выборок и хранения исторических прогнозов. Благодаря колоночному хранению обеспечивает мгновенный доступ к статистике за год.

3.  **Feature Store (Хранилище признаков)**
    *   **Назначение:** Промежуточное звено между БД и ИИ-моделью. 
    *   **Функции:** Хранение заранее вычисленных признаков (лаги продаж, скользящие средние, флаги промо). Это исключает повторные тяжелые вычисления при каждом запросе прогноза и гарантирует согласованность данных при обучении и инференсе.

4.  **ML Engine (Ядро обучения)**
    *   **Назначение:** Оркестрация жизненного цикла моделей машинного обучения.
    *   **Функции:** Запуск процесса переобучения на базе CatBoost/PySpark, подбор гиперпараметров, оценка качества моделей (Backtesting) и сохранение готовых артефактов моделей в реестр.

5.  **Inference API (Микросервис предсказаний)**
    *   **Назначение:** "Лицо" системы для потребителей. Высокодоступный шлюз, выдающий предсказания в реальном времени.
    *   **Функции:** Принимает запросы от ERP или менеджера, достает признаки из Feature Store, прогоняет их через модель и возвращает результат в формате "Дата — Товар — Прогноз".

6.  **ERP Connector (Адаптер интеграции)**
    *   **Назначение:** Обеспечение совместимости между современным ИИ-стеком и унаследованными (legacy) системами управления сетью.
    *   **Функции:** Конвертация прогнозов из JSON в формат, понятный ERP-системе (например, XML или CSV), и автоматическое создание документов "Заказ поставщику".

---

#### Методы API (Inference API)

Для взаимодействия со слоем приложений (Application Layer) сервис **Inference API** предоставляет следующие ключевые методы:

| Метод | Путь (Endpoint) | Назначение |
| :--- | :--- | :--- |
| **GET** | `/v1/forecast/store/{store_id}` | Получить полный прогноз на 14 дней для всех товаров конкретного магазина. Используется для формирования ежедневных автозаказов. |
| **GET** | `/v1/forecast/product/{id}` | Получить прогноз спроса для конкретного SKU. Параметры: `store_id`, `date_start`, `date_end`. Возвращает прогноз и доверительный интервал. |
| **POST** | `/v1/feedback/adjustment` | Метод для приема правок от менеджеров. Если менеджер изменил заказ вручную, система логирует это действие как сигнал для будущего дообучения (учет внешних факторов). |
| **GET** | `/v1/monitoring/health` | Возвращает статус готовности модели и актуальные метрики точности (WAPE/Bias) за последние сутки для административной панели. |

### 4.2. Описание инфраструктуры и масштабируемости

**Выбранная инфраструктура:**
Для проекта выбрана **распределенная облачная (или гибридная) архитектура** на базе колоночной СУБД **ClickHouse** для хранения данных и кластера **Kubernetes (K8s)** для развертывания микросервисов. Обработка данных (ETL) и обучение моделей производятся с помощью распределенных вычислений **Apache Spark**.

**Плюсы и минусы выбора:**
*   **Плюсы:** Высокая скорость выполнения аналитических запросов над миллиардами строк (2,67 млрд транзакций). Горизонтальная масштабируемость: при увеличении сети до 1000+ магазинов достаточно добавить новые узлы в кластер без переписывания кода. Изоляция процессов обучения (ML Engine) и выдачи прогнозов (Inference API).
*   **Минусы:** Высокая техническая сложность настройки и поддержки распределенной инфраструктуры. Необходимость в квалифицированных DevOps/Data-инженерах. Более высокие начальные затраты на облачные ресурсы по сравнению с классическими серверами.

**Почему финальный выбор лучше альтернатив:**
Классические реляционные базы данных (например, PostgreSQL) не способны обрабатывать агрегации по 2,6 млрд строк в реальном времени — запросы занимали бы часы вместо секунд. Решение на локальных серверах в каждом магазине невозможно, так как для качественного прогноза ИИ нужны глобальные данные по всей сети. Выбранный стек (ClickHouse + Spark + K8s) является отраслевым стандартом для Big Data проектов, обеспечивая баланс между стоимостью владения и производительностью.


### 4.3. Требования к работе системы

*   **SLA (Service Level Agreement):** Уровень доступности системы должен составлять **99.9%**. Это критично, так как отсутствие прогноза к 07:00 утра блокирует процесс заказа товаров, что ведет к пустым полкам в 500 магазинах.
*   **Пропускная способность (Throughput):** 
    *   *Слой данных:* обработка инкрементальной загрузки (~7-10 млн новых транзакций в сутки) в течение 30 минут. 
    *   *Слой прогноза:* расчет 5 млн индивидуальных прогнозов (500 магазинов × 10 000 товаров) в рамках 4-часового регламентного окна.
*   **Задержка (Latency):** Время ответа **Inference API** при запросе менеджера через интерфейс не должно превышать **200 мс** на одну позицию или **2 секунды** на полную витрину магазина (с использованием кэширования в Redis).


### 4.4. Безопасность системы

**Потенциальные уязвимости системы:**
1.  **Инъекции данных (Data Poisoning):** Риск попадания в обучающую выборку некорректных или специально сфабрикованных данных (например, фиктивных продаж), что может «испортить» модель и привести к массовым ошибочным заказам.
2.  **Доступ к API:** Публичные эндпоинты Inference API могут стать целью атаки или несанкционированного доступа, что приведет к утечке коммерческой информации о планах закупок и выручке сети.
3.  **Уязвимости в Open Source библиотеках:** Использование сторонних ML-библиотек (Python-пакеты) требует регулярного сканирования на наличие известных уязвимостей (CVE), так как через них возможен взлом серверной части.

**Методы защиты:** Для нейтрализации рисков используются строгая аутентификация микросервисов (mTLS), автоматизированные тесты на аномалии в данных (Data Validation) и изоляция вычислительного контура в частном облаке (VPC) без прямого доступа из интернета.
